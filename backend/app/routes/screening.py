...
def _enrich_result_out(item: ResultOut, resume: Optional[Resume] = None) -> ResultOut:
    """Attach display fields from a recruiter-uploaded Resume (no candidate_id on Resume)."""
    if resume:
        item.candidate_name = resume.candidate_name or ""
        item.candidate_email = resume.email or ""
        item.candidate_id = ""
        item.filename = resume.filename or ""
    return item
    ...
    for resume in resumes:
        try:
            ...
            result = Result(
                ...
                strength_tags=result_data.get("strength_tags", []),
                status="pending",
            )
            db.add(result)
            db.commit()
...
def get_results(
    ...
    for r in query.all():
        ...
        out.append(_enrich_result_out(item, r.resume))
...
def update_result(
    ...
    if result:
        ...
        return _enrich_result_out(item, result.resume)
